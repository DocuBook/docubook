...
Open fullscreen by clicking the button, or focus the container with <kbd>Tab</kbd> then press <kbd>Enter</kbd>. Fullscreen locks page scroll and shows:

- **Pan** — mouse-drag, touch-drag, or arrow keys
- **Zoom in / zoom out** — scroll wheel, <kbd>+</kbd> / <kbd>-</kbd> keys, or the zoom bar buttons (range 0.4× to 4×)
- **Reset** — click the zoom percentage button or press <kbd>0</kbd>
- **Help (?)** — auto-opens on first fullscreen entry, re-openable anytime via the `?` button
- **Exit** — Exit fullscreen button (top-right) or <kbd>Escape</kbd>
...
- **Pan, zoom, and fullscreen**: A fullscreen button appears after the diagram renders. Click or press Enter to open the fullscreen lightbox. Inside fullscreen, pan with mouse-drag, touch-drag, or arrow keys; zoom with the zoom bar buttons, scroll wheel, or <kbd>+</kbd> / <kbd>-</kbd>; reset via the zoom percentage button or <kbd>0</kbd>. Press Escape to exit.
