...
Generates:
- `Dockerfile` — multi-stage build pinned to the installed `@docubook/flame` version
- `nginx.conf` — security headers, cache tiers, clean URLs, and the generated `404.html`
- `.dockerignore` — optimized for build speed

The exact builder image tag keeps local CLI output and Docker builds on the same asset-manifest contract. Upgrade `@docubook/flame`, then rerun `flame deploy --docker` when you want a newer builder. The final output is a nginx container serving flat `.html` files with clean URLs.
