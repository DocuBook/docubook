...
| ------- | ------- | ------- |
| `meta.basePath` | Prefix **inside** the build output (default `/docs`) | `/docs`, `/handbook`, `""` |
| `meta.baseURL` | Origin **plus deployment root** — the path the host publishes the dist under | `https://acme.github.io/handbook` |
...
Do not repeat the docs prefix in `baseURL` — it is appended automatically when canonical and Open Graph URLs are built. See [Base Path & Subpaths](/docs/getting-started/base-path) for root deployments, canonicalization, and verification steps.
...
Project sites are published under the repository name, so `meta.baseURL` has to name that path (`https://<user>.github.io/<repo>`) — see [Serving Under a Subpath](#serving-under-a-subpath).
...
Generates:
- `Dockerfile` — multi-stage build tailored to the project's package manager
- `nginx.conf` — security headers, cache tiers, clean URLs, and the generated `404.html`. The content-assets cache block follows `meta.basePath`; at a root deployment (`"basePath": ""`) it is omitted because both asset trees would claim `/assets/`.
...
- `flame` CLI — run via `bunx flame`, `npx flame`, or install globally with `bun install -g @docubook/flame`
...
|----------|---------------|
| Coolify | Connect repo, set build pack to Dockerfile. Or pull pre-built image from GHCR. |
| Fly.io | `fly deploy` — auto-detects Dockerfile in repo. |
| Railway | Auto-detects Dockerfile, deploys on push. |
