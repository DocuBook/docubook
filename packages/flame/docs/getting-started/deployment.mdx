...
The generated Dockerfile uses a **major version tag** (`:1`, `:2`, etc.) which means patch and minor updates are automatic on rebuild. The final output is a nginx container serving flat `.html` files with clean URLs.
