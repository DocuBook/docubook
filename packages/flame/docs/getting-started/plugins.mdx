...
:::step{title="Step 3: Using Factory Options"}
...
:::step{title="Step 4: Managing Plugin State"}
...
:::step{title="Step 5: Error Handling"}
...
:::step{title="Step 6: Testing Plugins"}
...
:::step{title="Step 7: Using Multiple Hooks"}
...
:::step{title="Step 8: Publishing a Plugin"}
...
### onEnd(config, pages, context)

Called after all page HTML has been written. Generate sitemaps, RSS feeds, or other output using the final output directory and page-specific asset manifest.

```typescript
build.onEnd((config, pages, { outDir, assetManifest }) => {
  // pages: { slug, title, filePath, outputPath }[]
  console.log(`Built ${pages.length} pages to ${outDir}`);
  console.log(`Docs CSS: ${assetManifest.docs.css}`);
});
```
...
Return HTML strings to inject inside `<head>`. Results from all plugins are merged and deduplicated. This hook runs for docs, home, and standalone 404 pages.

```typescript
build.injectHead((ctx) =>
  `<meta name="docubook-page-type" content="${ctx.pageType}">`
);
```

### injectBody(ctx)

Return HTML strings to inject before `</body>`. Check `ctx.assets.js` before generating markup that depends on a client bundle because JavaScript is optional, including on the standalone 404 page.

```typescript
build.injectBody((ctx) =>
  ctx.assets.js ? `<script>console.log("Client bundle loaded")</script>` : []
);
```

### transformHtml(html, ctx)

Transform the final HTML string for docs, home, and standalone 404 pages. It is the last page hook before writing HTML to disk.

```typescript
build.transformHtml((html, ctx) =>
  html.replace("</body>", `<!-- ${ctx.pageType} -->\n</body>`)
);
```

### Page context and assets

`injectHead`, `injectBody`, and `transformHtml` receive a `PageContext`:

| Field | Description |
| ----- | ----------- |
| `pageType` | `"docs"`, `"home"`, or `"notFound"` |
| `assets` | Hashed `{ css, js? }` entry used by this page |
| `slug` | Docs slug, an empty string for home, or `"404"` |
| `filePath` | Source or framework page path |
| `frontmatter` | Docs frontmatter, site metadata for home, or an empty object for 404 |
| `content` | Raw MDX/Markdown content when available |
| `config` | Resolved DocuBook configuration |

The final asset manifest uses the same page families:

```typescript
{
  docs: { css: "docs-HASH.css", js: "client-HASH.js" },
  home: { css: "site-HASH.css", js: "home-client-HASH.js" },
  notFound: { css: "site-HASH.css" }
}
```

Do not parse `manifest.json` as a single `{ js, css }` bundle. Read the entry for the current `pageType`, and treat every `js` field as optional.

### handleRequest(req, ctx)

Intercept incoming dev server requests. First plugin to return a `Response` short-circuits. The context contains `port`, `hostname`, and the current page-specific `assetManifest`, including updates from dev rebuilds.
...
build.handleRequest((req, ctx) => {
  ...
  if (url.pathname === "/api/status") {
    return Response.json({
      ok: true,
      docsCss: ctx.assetManifest.docs.css,
    });
  }
});
```
...
│   ├── injectHead() / injectBody()         ← docs context
│   ├── transformHtml()                     ← final docs transform
│   └── write docs HTML
├─ render home → page hooks → write HTML
├─ render standalone 404 → page hooks → write HTML
└─ onEnd(config, pages, context)             ← final output metadata
```

Execution is **sequential** (waterfall). Plugins run in `docu.json` order and `handleRequest` is **first-wins**. Execution-hook errors are logged and isolated; collection hooks such as `injectHead` and `injectBody` throw developer errors.

## Example Plugins

Plugin hooks run on Bun, Node, and Deno. For a plugin that works everywhere, use `node:` APIs (e.g. `node:fs/promises`). If you want Bun's faster `Bun.write` when it is available, guard it behind a `typeof Bun !== "undefined"` check and fall back to `writeFile` on Node and Deno — the sitemap plugin below does exactly that. Plugins that will only ever run in a Bun-only project can call `Bun.write` directly.
