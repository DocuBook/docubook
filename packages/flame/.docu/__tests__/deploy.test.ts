...
import { detectPkgManager, generateDockerfile } from "../node/deploy.shared";
...
describe("generateDockerfile — package manager aware", () => {
  let tmpDir: string;

  beforeEach(async () => {
    tmpDir = join(tmpdir(), `deploy-dockerfile-${Date.now()}`);
  ...
  it("keeps the zero-config builder image for bun projects", async () => {
    await writeFile(join(tmpDir, "package.json"), "{}");
    await writeFile(join(tmpDir, "bun.lock"), "");
    expect(generateDockerfile(tmpDir)).toBe(DOCKERFILE_BUN);
  });

  it("installs npm dependencies before building an npm project", async () => {
    await writeFile(join(tmpDir, "package.json"), "{}");
    await writeFile(join(tmpDir, "package-lock.json"), "{}");
    const dockerfile = generateDockerfile(tmpDir);
    expect(dockerfile).toContain("# Generated by @docubook/flame deploy --docker");
  ...
  it("copies no lockfile and uses a plain install when none exists", async () => {
    await writeFile(join(tmpDir, "package.json"), "{}");
    const dockerfile = generateDockerfile(tmpDir, "");
  ...
  it("uses the invoking package manager when no lockfile exists", async () => {
    await writeFile(join(tmpDir, "package.json"), "{}");
    const dockerfile = generateDockerfile(tmpDir, "pnpm/10.0.0 npm/? node/v22");
    expect(dockerfile).toContain("FROM node:22-alpine AS builder");
    expect(dockerfile).toContain("COPY package.json ./");
  ...
  it("prefers the lockfile over the invoking package manager", async () => {
    await writeFile(join(tmpDir, "package.json"), "{}");
    await writeFile(join(tmpDir, "bun.lock"), "");
    expect(generateDockerfile(tmpDir, "pnpm/10.0.0 npm/? node/v22")).toBe(DOCKERFILE_BUN);
  });

  it("installs pnpm dependencies via corepack", async () => {
    await writeFile(join(tmpDir, "package.json"), "{}");
    await writeFile(join(tmpDir, "pnpm-lock.yaml"), "");
    const dockerfile = generateDockerfile(tmpDir);
    expect(dockerfile).toContain("COPY package.json pnpm-lock.yaml ./");
    expect(dockerfile).toContain("corepack enable && pnpm install --frozen-lockfile --prod=false");
    expect(dockerfile).toContain("RUN pnpm run build");
  });
});
...
describe("detectPkgManager — lockfile detection", () => {
  ...
  it("falls back to the invoking package manager when no lockfile exists", async () => {
    await touch("package.json");
    expect(detectPkgManager(tmpDir, "yarn/1.22.22 npm/? node/v22").runCmd).toBe("yarn");
    expect(detectPkgManager(tmpDir, "bun/1.4.0").runCmd).toBe("bun");
    expect(detectPkgManager(tmpDir, "").runCmd).toBe("npm");
  });

  it("ignores the invoking package manager when a lockfile exists", async () => {
    await touch("pnpm-lock.yaml");
    await touch("package.json");
    expect(detectPkgManager(tmpDir, "bun/1.4.0").runCmd).toBe("pnpm");
  });
});
