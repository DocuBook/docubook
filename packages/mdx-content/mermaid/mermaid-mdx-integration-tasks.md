...
| T-008 | Create `rehypeMermaid` plugin in `@docubook/core` | Create `packages/core/src/plugins/rehypeMermaid.ts`: walk AST for `<pre><code class="language-mermaid">`, extract text content, replace with `<Mermaid chart="...">`. Export from `plugins/index.ts`, register in `createDefaultRehypePlugins()` in `compile.ts` | S | T-001 | ` ```mermaid ` compiled MDX renders `<Mermaid chart="...">` element instead of `<pre><code>` | Design §5 |
