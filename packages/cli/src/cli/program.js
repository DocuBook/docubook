  ...
  program
    ...
    .action(async () => {
      ...
      try {
        ...
        if (!res.ok) throw new Error(`Failed to fetch registry metadata (status ${res.status})`);
        const data = await res.json();
        const latest = data && data["dist-tags"] && data["dist-tags"].latest;
        if (!latest) throw new Error("Could not determine latest version from npm registry");
