...
> Pay attention to the <span className="text-red-500 font-bold">highlighted</span> parts of the crawler syntax below—these are the sections you
> need to modify.

<Accordion title="crawler editor">
```javascript:index.js showLineNumbers {2,5,12,15-16,62,113}
new Crawler({
  appId: "5CCY***", // changes appId
  indexPrefix: "",
  rateLimit: 8,
  startUrls: ["https://www.docubook.pro"], // changes to your domain
  ...
  discoveryPatterns: ["https://www.docubook.pro/**"], // changes to your domain
  actions: [
    {
      indexName: "docs.crawler", // changes crawler name
      pathsToMatch: ["https://www.docubook.pro/**"], // changes to your domain
        ...
        return helpers
          ...
          .map((record) => {
            ...
            return {
              ...record,
              anchor: undefined,
              url: record.url ? record.url.replace(/#(scroll-container|main-navbar)$/, "") : record.url,
              url_without_anchor:
                record.url_without_anchor || (record.url ? record.url.split("#")[0] : record.url),
            };
          });
      },
    },
  ...
  initialIndexSettings: {
    "docs.crawler": { // changes crawler name
  ...
  apiKey: "01469***", // changes crawler apiKey
});
```
</Accordion>
