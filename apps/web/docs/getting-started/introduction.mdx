...
> Note: The current optimization uses two cache layers (`getDocsRawForSlugCached` and `computeDocsForSlugCached`) so metadata, page content, and TOC all share the same raw MDX processing in one request.
